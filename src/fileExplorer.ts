...
export class FileSystemProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	async getChildren(element?: Entry): Promise<Entry[]> {
		console.log("debugging. getChildren called");
		...
		if (element) {
			...
			try
			{
				...
				return children.map(([name, type]) => ({ uri: Utils.joinPath(folderuri, name), type }));
				}
			catch(e)
			{
		...
		if (workspaceFolder) {
			const temparr = await this.readDirectory(workspaceFolder.uri);

			const children = temparr.filter(function( obj ) {
				return obj[0].endsWith(".dmn");
			});
			...
			console.log("getting ws children of " + children.toString());
